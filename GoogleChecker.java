...
import lnc.paym.Begain.Login;
import lnc.paym.CustomView.CustomAlert.UpdateAlert;
import lnc.paym.R;
...
public class GoogleChecker {
    ...
    public void AlertDialog(int message, String version, Context c, boolean haveno, DialogInterface.OnClickListener listener) {
        UpdateAlert updateAlert=new UpdateAlert(c);
        updateAlert.setMessage(c.getResources().getString(R.string.youAreNotUpdatedMessage)+" "+version);
        updateAlert.setCancelable(false);
        updateAlert.setPositveButton(c.getResources().getString(R.string.update_ckeck), new View.OnClickListener() {
            @Override
        ...
        alertDialog.setMessage(c.getResources().getString(message)+" "+version);

        alertDialog.setPositiveButton(R.string.update, listener);
        if (haveno) {
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if (isThereNewVersion) {
                                ...
                                AlertDialog(R.string.youAreNotUpdatedMessage,marketVersion, context, haveno, new DialogInterface.OnClickListener() {
                                    @Override
                                    public void onClick(DialogInterface dialog, int which) {
